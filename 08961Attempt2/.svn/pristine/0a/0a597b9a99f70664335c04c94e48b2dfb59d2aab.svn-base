...
TextureShader::TextureShader(void):m_vertexShader(0), m_pixelShader(0), m_layout(0), m_matrixBuffer(0), m_sampleState(0)
{
}
...
{
	bool result;
	result = InitializeShader(device, hwnd,  L"../Engine/Texture.vs", L"../Engine/RayMarchPS.hlsl");
	if(!result)
	{
		Logger::AppendErrorToLog("InitializeShader failed in TextureShader::Initialize");
		return false;
	}
	return true;
}
...
bool TextureShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, const XMMATRIX & worldMatrix, const XMMATRIX & viewMatrix, const XMMATRIX & projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	if(!result)
	{
		Logger::AppendErrorToLog("SetShaderParameters failed at TextureShader::Render");
		return false;
	}
...
bool TextureShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	//hold description of texture sampler
	...
	result = D3DCompileFromFile (psFilename, NULL, NULL, "RaymarchPixelShader", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		if(errorMessage)
	...
	//filter is VERY important. Detirmines how pixels will be used or combined to create the final look of the texture.
...
// texture has to be set before rendering of the buffer occurs.
bool TextureShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX & p_worldMatrix, const XMMATRIX & p_viewMatrix, 
					     const XMMATRIX & p_projectionMatrix, ID3D11ShaderResourceView* texture)
{
	HRESULT result;
	...
	//The SetShaderParameters function has been modified to include setting the texture in the pixel shader now. 
