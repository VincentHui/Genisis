...
LightShader::LightShader(void): m_vertexShader(0),
	m_pixelShader(0),
	m_layout(0),
	m_sampleState(0),
	m_matrixBuffer(0),
...
{
	bool result;
	// new light.vs and light.ps HLSL shaders are used
...
{
	ShutdownShader();
	return; //? wtf rastertek
}

bool LightShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMFLOAT4X4 worldMatrix, XMFLOAT4X4 viewMatrix, 
						 XMFLOAT4X4 projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT3 lightDirection, 
						 XMFLOAT4 diffuseColor, XMFLOAT4 ambient, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
	bool result;
	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, lightDirection, diffuseColor, ambient,
...
bool LightShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	//polygon layout has been made larger compared to other classes
	//to accomadate the new normal vector in the layout 
	D3D11_INPUT_ELEMENT_DESC polygonLayout[3];
	unsigned int numElements;
	D3D11_SAMPLER_DESC samplerDesc;
	D3D11_BUFFER_DESC matrixBufferDesc;
	D3D11_BUFFER_DESC cameraBufferDesc;
	/*also added new lightBuffer Desc*/ D3D11_BUFFER_DESC lightBufferDesc;
	...
	//setup light constant buffer description
...
bool LightShader::SetShaderParameters(ID3D11DeviceContext* p_DeviceContext, XMFLOAT4X4 p_worldMatrix, XMFLOAT4X4 p_viewMatrix, 
					   XMFLOAT4X4 p_projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT3 lightDirection, 
					   XMFLOAT4 diffuseColor, XMFLOAT4 ambient, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, 
					   float specularPower)
{
	HRESULT result;
	...
	// Copy the matrices into the constant buffer.
	dataPtr->world = _4X4worldMatrix;
	dataPtr->view = _4X4viewMatrix;
	dataPtr->projection = _4X4projectionMatrix;
	...
	p_DeviceContext->PSSetShaderResources(0, 1, &texture);

	//setting up the light constant buffer
	//setup the same way as the matrix buffer
	//uses PSSetConstantBuffers function instead
