...
HLSL_invokerTess::HLSL_invokerTess(void)
{
...
bool HLSL_invokerTess::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"../Engine/LightVS.hlsl", L"../Engine/LightHS.hlsl", L"../Engine/LightDS.hlsl", L"../Engine/LightPS.hlsl");
	...
	if(!result)
	{
		Logger::AppendErrorToLog("InitializeShader in HLSL_invokerTess failed");
		return false;
	}
	return true;
}

void HLSL_invokerTess::Shutdown()
{
...
bool HLSL_invokerTess::Render(ID3D11DeviceContext* deviceContext, int indexCount,  XMFLOAT4X4  worldMatrix, 
				   ...
				   const LightInfo& LightInfo)
{
	...
	if(!result)
	{
		Logger::AppendErrorToLog("SetShaderParameters in HLSL_invokerTess failed");
		return false;
	}
	...
	bool HLSL_invokerTess::InitializBezierShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* hsFilename, WCHAR* dsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	TesDisplacementBufferDesc.ByteWidth = sizeof(TessellationBufferType);
...
bool HLSL_invokerTess::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* hsFilename, WCHAR* dsFilename, WCHAR* psFilename)
{
	HRESULT result;
	ID3D10Blob* errorMessage;
	ID3D10Blob* vertexShaderBuffer;
	...
	// Compile the hull shader code.
	result = D3DCompileFromFile(hsFilename, NULL, NULL, "LightHullShader", "hs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,  
				       &hullShaderBuffer, &errorMessage);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
		...
		// Compile the domain shader code.
	result = D3DCompileFromFile(dsFilename, NULL, NULL, "LightDomainShader", "ds_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,  
				       &domainShaderBuffer, &errorMessage);
	if(FAILED(result))
	{
		// If the shader failed to compile it should have writen something to the error message.
...
void HLSL_invokerTess::ShutdownShader()
{
...
void HLSL_invokerTess::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
// this function is called to send them into the vertex shader during the Render call in HLSL_invokerTess. 
bool HLSL_invokerTess::SetShaderParameters(ID3D11DeviceContext* p_DeviceContext,  XMFLOAT4X4 p_worldMatrix,  XMFLOAT4X4 p_viewMatrix,  XMFLOAT4X4 p_projectionMatrix, 
										   float p_tessfactor, float p_tessBias, float p_tessScale
										   , ID3D11ShaderResourceView** p_textureArray, const XMFLOAT3 & p_camPos, const XMFLOAT4 & p_tess_misc, const LightInfo& LightInfo)
{
	HRESULT result;
	...
	if(FAILED(result))
	{
		Logger::AppendErrorToLog("p_DeviceContext couldn't Map, at HLSL_invokerTess");
		return false;
	}
	...
	p_DeviceContext->DSSetShaderResources(1, 1, &p_textureArray[1]);
	p_DeviceContext->PSGetShaderResources(0, 1, &p_textureArray[0]);
...
void HLSL_invokerTess::RenderShader(ID3D11DeviceContext* p_deviceContext, int p_indexCount)
{
