...
void D3DClass::Resize(HWND wnd, UINT message, WPARAM wParam, LPARAM lParam)
{

	if (this->GetDevice()) // Resize D3D render target
    {
		ID3D11Device * g_D3DDev = this->GetDevice();
		ID3D11DeviceContext * g_D3DDevCtx = this->GetDeviceContext();
        ...
        if (m_swapChain)
        {
			IDXGISwapChain* g_SwapChain = m_swapChain;
			DXGI_SWAP_CHAIN_DESC g_SwapChainDesc = m_SwapChainDesc;
            // Resize swap chain
            ...
            g_D3DDev->CreateRenderTargetView(backBuffer, NULL, &m_renderTargetView);
            backBuffer->Release();
            ...
            g_D3DDev->CreateDepthStencilView(dsBuffer, NULL, &m_depthStencilView);
            dsBuffer->Release();
            g_D3DDevCtx->OMSetRenderTargets(1, &m_renderTargetView, m_depthStencilView);
						  ...
						  float screenDepth, float screenNear)
{
	...
	//string memory = "memory: " + m_videoCardMemory;
	//m_logger.AppendToLog(memory);

	Logger::AppendInt(m_videoCardMemory, "memory ");

	Logger::AppendToLog("video card: " + static_cast<string>(m_videoCardDescription));
	...
	g_DepthStencilDesc.SampleDesc = m_SwapChainDesc.SampleDesc;
	...
	result = m_device->CreateTexture2D(&g_DepthStencilDesc, NULL, &dsBuffer);
	...
	result = m_device->CreateDepthStencilView(dsBuffer, NULL, &m_depthStencilView);
    dsBuffer->Release();
    if (FAILED(result))
	{
        return false;
	...
	m_projectionMatrix = XMMatrixPerspectiveFovLH ( fieldOfView, aspectRatio, screenNear, screenDepth);
	...
	m_orthoMatrix = XMMatrixOrthographicLH((float)width, (float)height, screenNear, screenDepth /*z far value?*/);
